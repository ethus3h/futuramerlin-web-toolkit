...
if ( ! function_exists( 'fmwtkwordpress_posted_on' ) ) :
	...
	function fmwtkwordpress_posted_on() {
		...
		$byline = sprintf(
			...
			__( 'by %s', 'fmwtkwordpress' ),
			'<span class="author vcard"><a class="url fn n" href="' . esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) . '">' . get_the_author() . '</a></span>'
		);

		// Finally, let's write all of this to the page.
		echo '<span class="posted-on">' . fmwtkwordpress_time_link() . '</span><span class="byline"> ' . $byline . '</span>';
	}
endif;
...
if ( ! function_exists( 'fmwtkwordpress_time_link' ) ) :
	...
	function fmwtkwordpress_time_link() {
		...
		return sprintf(
			...
			__( '<span class="screen-reader-text">Posted on</span> %s', 'fmwtkwordpress' ),
			...
			if ( 'post' === get_post_type() ) {
				if ( ( $categories_list && fmwtkwordpress_categorized_blog() ) || $tags_list ) {
					...
					if ( $categories_list && fmwtkwordpress_categorized_blog() ) {
						echo '<span class="cat-links">' . fmwtkwordpress_get_svg( array( 'icon' => 'folder-open' ) ) . '<span class="screen-reader-text">' . __( 'Categories', 'fmwtkwordpress' ) . '</span>' . $categories_list . '</span>';
					}

					if ( $tags_list && ! is_wp_error( $tags_list ) ) {
						echo '<span class="tags-links">' . fmwtkwordpress_get_svg( array( 'icon' => 'hashtag' ) ) . '<span class="screen-reader-text">' . __( 'Tags', 'fmwtkwordpress' ) . '</span>' . $tags_list . '</span>';
					}
 ...
 */
function fmwtkwordpress_front_page_section( $partial = null, $id = 0 ) {
	if ( is_a( $partial, 'WP_Customize_Partial' ) ) {
		// Find out the id and set it up during a selective refresh.
		global $fmwtkwordpresscounter;
		$id                     = str_replace( 'panel_', '', $partial->id );
		$fmwtkwordpresscounter = $id;
	}
	...
	if ( get_theme_mod( 'panel_' . $id ) ) {
		...
		echo '<article class="panel-placeholder panel fmwtkwordpress-panel fmwtkwordpress-panel' . $id . '" id="panel' . $id . '"><span class="fmwtkwordpress-panel-title">' . sprintf( __( 'Front Page Section %1$s Placeholder', 'fmwtkwordpress' ), $id ) . '</span></article>';
	}
}
