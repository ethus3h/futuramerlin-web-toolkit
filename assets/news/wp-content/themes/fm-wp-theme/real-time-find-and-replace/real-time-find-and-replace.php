...
Author URI: http://futuramerlin.com/ http://infolific.com/technology/
License: GPLv2 or later
Text Domain: fm-real-time-find-and-replace
*/
...
function far_plugin_meta( $links, $file ) { // add some links to plugin meta row
	if ( strpos( $file, 'real-time-find-and-replace.php' ) !== false ) {
		...
		$links = array_merge( $links, array( '<a href="http://infolific.com/technology/software-worth-using/real-time-find-and-replace-for-wordpress/#pro-version" target="_blank">Pro Version</a>' ) );
	}
	return $links;
}
				...
				if ( isset ( $far_settings['farfind'] ) && is_array( $far_settings['farfind'] ) ){
					...
					foreach ( $far_settings['farfind'] as $key => $find ){
						...
						echo "<div style='float: left'>";
							...
							echo "<input class='checkbox' type='checkbox' name='faradmin[$i]' id='faradmin$i' />";
							echo "&nbsp;&nbsp;";
							echo "<label class='side-label-long' for='faradmin$i'>Ignore Case:&nbsp;</label>";
							echo "<input class='checkbox' type='checkbox' name='farcaseinsensitive[$i]' id='farcaseinsensitive$i' />";
							echo "<br />";
							...
							echo "<input class='textbox' type='text' name='farquerystring[$i]' id='farquerystring$i' value='' />";
							echo "<br />";

							echo "<label class='side-label' for='farreferrer$i'>Referrer:</label>";
							echo "<input class='textbox' type='text' name='farreferrer[$i]' id='farreferrer$i' value='' />";
							echo "<br />";

							echo "<label class='side-label' for='faruseragent$i'>User Agent:</label>";
							echo "<input class='textbox' type='text' name='faruseragent[$i]' id='faruseragent$i' value='' />";
							echo "<br />";

						echo "</div>";

						echo "<div>";
							echo "<label class='side-label' for='fardescription$i'>Description:</label>";
							echo "<input style='width: 615px;' type='text' name='fardescription[$i]' id='fardescription$i' value='' />";
	...
	<div id="far-sb">
		<div class="postbox" id="far-sbone">
			...
			<div class="inside">
				...
				<li>It may be possible to control when the replace occurs by specifying a querytring, referral, and post type to match. Note that these filters perform a simple "contains" for the text given.</li>
				</ol>
				<strong>Tips</strong>
				<ol>
...
function far_ob_call($buffer) { // $buffer contains entire page
	//todo: implement various features
